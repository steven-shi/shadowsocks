...
__homepage__ = 'https://github.com/abhinavsingh/proxy'
__license__ = 'BSD'
    ...
    def _process_request(self, data):
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            if self.request.method == b"CONNECT":
                ...
                host, port = self.request.url.hostname or self.request.headers['host'][1],\
...
def main():
    parser = argparse.ArgumentParser(
        description='proxy v%s' % __version__,
        epilog='Having difficulty using proxy? Report at: %s/issues/new' % __homepage__
    )
